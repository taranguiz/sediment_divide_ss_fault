...
BUNDLE_NAME="prr_tabular_outputs_intermediate_slip_rates"
...
for model in "${MODELS[@]}"; do
  src="output/${model}/tabular_outputs"
...
if [[ "$missing" -ne 0 ]]; then
  echo "Stopped: $missing tabular_outputs folder(s) were missing."
  echo "Nothing was archived. Check the missing model output folders above."
  exit 1
fi

{
  echo "PRR tabular outputs bundle for intermediate slip-rate runs"
  echo
