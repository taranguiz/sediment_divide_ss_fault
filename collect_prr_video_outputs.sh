...
BUNDLE_NAME="prr_video_outputs_all_families"
...
for model in "${MODELS[@]}"; do
  src="output/${model}/${model}_evolution.mp4"
...
if [[ "$missing" -ne 0 ]]; then
  echo "Stopped: $missing video file(s) were missing."
  echo "Nothing was archived. Check the missing model output folders above."
  exit 1
fi

{
  echo "PRR video outputs bundle for all families"
  echo
